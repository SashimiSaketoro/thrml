...
pub trait StateLeaf: Clone {
    fn stack(leaves: &[Self]) -> Self where Self: Sized;
...
pub fn from_global_state<L: StateLeaf>(
    ...
    for block in blocks_to_extract {
        let (sd_ind, slices) = spec_from.get_node_locations(block)
        ...
        let indices: Tensor<WgpuBackend, 1, burn::tensor::Int> = 
            Tensor::from_data(slices.iter().map(|&x| x as i32).collect::<Vec<_>>().as_slice(), device);
...
pub fn make_empty_block_state(
    ...
    for block in blocks {
        let _spec = node_shape_dtypes.get(block.node_type())
