...
impl BlockSpec {
    pub fn new(blocks: Vec<Block>, node_shape_dtypes: IndexMap<NodeType, TensorSpec>) -> Result<Self, String> {
        ...
        for (block_idx, block) in blocks.iter().enumerate() {
            ...
            let spec = node_shape_dtypes.get(block.node_type())
                .ok_or_else(|| format!("Node type {:?} not found in node_shape_dtypes", block.node_type()))?;
    ...
    pub fn get_node_locations(&self, block: &Block) -> Result<(usize, Vec<usize>), String> {
        let node_sds = self.node_shape_dtypes.get(block.node_type())
            .ok_or_else(|| format!("Node type {:?} not found", block.node_type()))?;
        let sd_inds = *self.sd_index_map.get(node_sds)
            .ok_or_else(|| "SD not found in index map".to_string())?;
        let global_locs: Vec<usize> = block.nodes().iter()
            .map(|node| {
                self.node_global_location_map.get(node)
                    .map(|(_, pos)| *pos)
