...
use thrml_models::ising::{IsingEBM, hinton_init};
use thrml_models::AbstractEBM;
...
fn main() {
    ...
    let weights: Tensor<WgpuBackend, 1> = Tensor::from_data(
        vec![coupling_strength; edges.len()].as_slice(),
        &device,
    );
    ...
    let all_nodes_block = Block::new(nodes.clone())
    ...
    println!("Sampling schedule: {} warmup, {} samples, {} steps/sample\n",
    ...
    let init_states = hinton_init(key, &model, &[all_nodes_block.clone()], &batch_shape, &device);
    
    println!("✓ Initial states generated using Hinton initialization\n");
    
    // Print initial state
    if let Some(first_state) = init_states.first() {
        let state_data: Vec<f32> = first_state.clone()
            .into_data()
            .to_vec()
            .expect("read state");
    ...
    println!("✓ Model has {} factors (bias + edge terms)\n", factors.len());
    ...
    let state_data: Vec<f32> = sample_state.clone().into_data().to_vec().expect("read state");
    println!("All-up state representation: {:?}\n", 
        state_data.iter()
            .map(|&x| if x > 0.5 { "↑" } else { "↓" })
            .collect::<Vec<_>>()
            .join(" ")
    ...
    println!("Alternating state representation: {:?}\n", 
        alt_state_data.iter()
            .map(|&x| if x > 0.5 { "↑" } else { "↓" })
            .collect::<Vec<_>>()
            .join(" ")
