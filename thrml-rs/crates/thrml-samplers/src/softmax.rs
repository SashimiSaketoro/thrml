...
impl CategoricalGibbsConditional {
    pub fn new(n_categories: usize, n_spin: usize) -> Self {
        CategoricalGibbsConditional { n_categories, n_spin }
    }
}
...
fn split_states(
    ...
    let states_spin: Vec<Tensor<WgpuBackend, 2>> = states.iter()
        .take(n_spin)
        .cloned()
        .collect();
    
    let states_cat: Vec<Tensor<WgpuBackend, 2>> = states.iter()
        .skip(n_spin)
        .cloned()
        .collect();
...
fn spin_product_2d(
    ...
    let converted: Vec<Tensor<WgpuBackend, 2>> = spin_vals.iter()
...
mod tests {
    ...
    fn test_categorical_sample_basic() {
        ...
        let logits_1d: Tensor<WgpuBackend, 1> = Tensor::from_data(
        ...
        assert_eq!(samples_data[0] as i32, 2, "First sample should be category 2");
        assert_eq!(samples_data[1] as i32, 1, "Second sample should be category 1");
    }
}
