...
impl AbstractConditionalSampler for BernoulliConditional {
    fn sample(
            ...
            itertools::izip!(interactions, active_flags, neighbor_states)
        {
            ...
            let interaction_sum_2d: Tensor<WgpuBackend, 2> =
                interaction.clone().sum_dim(1).squeeze_dim(1);
            // Sum over dimension 1 (tail_dim), then squeeze -> [n_nodes]
            let interaction_sum: Tensor<WgpuBackend, 1> =
                interaction_sum_2d.sum_dim(1).squeeze_dim(1);
