...
pub fn run_blocks(
    ...
    for iter_key in keys {
        sample_blocks(iter_key, &mut init_chain_state, state_clamp, program, device)?;
    }
...
pub fn sample_with_observation<O: AbstractObserver>(
    ...
    let warmup_obs = warmup_observation.ok_or_else(|| {
        "Observer returned None for warmup observation".to_string()
    })?;
    ...
    if schedule.n_samples <= 1 {
        ...
        let warmup_with_dim: Vec<Tensor<WgpuBackend, 2>> = warmup_obs.into_iter()
