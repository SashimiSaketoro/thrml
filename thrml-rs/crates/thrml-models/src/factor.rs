    ...
    pub fn new(
        ...
        for block in &tail_nodes {
            if block.len() != interaction_size {
                return Err("All tail node blocks must have the same length as head_nodes".to_string());
            }
        }
        ...
        if weight_dims[0] != interaction_size {
            return Err(
                "Interaction weights must have leading dimension equal to the length of head_nodes".to_string()
            );
        }
...
pub fn validate_weights(weights: &Tensor<WgpuBackend, 3>, n_nodes: usize, n_categorical: usize) -> Result<(), String> {
