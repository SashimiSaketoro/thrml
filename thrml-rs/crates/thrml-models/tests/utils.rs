...
pub fn generate_all_states_categorical(
    ...
    for combo in (0..num_categorical).map(|_| 0..n_categories).multi_cartesian_product() {
        for &val in &combo {
...
pub fn generate_all_states_bin_cat(
    ...
    if total_states == 0 || (n_bin_states == 1 && bin_dims[1] == 0) && (n_cat_states == 1 && cat_dims[1] == 0) {
    ...
    let bin_data: Vec<f32> = bin_states.clone().into_data().to_vec().expect("read bin data");
    let cat_data: Vec<i32> = cat_states.clone().into_data().to_vec().expect("read cat data");
...
pub fn count_samples(
    ...
    let all_bin_data: Vec<f32> = all_states_bin.clone().into_data().to_vec().expect("read all bin");
    let all_cat_data: Vec<i32> = all_states_cat.clone().into_data().to_vec().expect("read all cat");
    let samples_bin_data: Vec<f32> = samples_bin.clone().into_data().to_vec().expect("read samples bin");
    let samples_cat_data: Vec<i32> = samples_cat.clone().into_data().to_vec().expect("read samples cat");
    ...
    for i in 0..n_all_states {
        let bin_state: Vec<u8> = (0..n_bin)
            ...
            .collect();
        let cat_state: Vec<i32> = (0..n_cat)
            .map(|k| all_cat_data[i * n_cat + k])
            .collect();
...
pub fn sample_and_compare_distribution<E: AbstractEBM>(
    ...
    let samples_cat = if !all_cat_nodes.is_empty() && all_block_samples.len() > (!all_binary_nodes.is_empty() as usize) {
    ...
    let (all_bin_states, all_cat_states) = generate_all_states_bin_cat(
        all_binary_nodes.len(),
        all_cat_nodes.len(),
        n_cats,
        device,
    );
    ...
    // Count samples
    let empirical_dist = count_samples(&all_bin_states, &all_cat_states, &samples_bin, &samples_cat, device);
    ...
    for i in 0..n_states {
        ...
        let bin_slice: Tensor<WgpuBackend, 2> = all_bin_states.clone().slice([i..i+1, 0..bin_dims[1]]);
        let bin_state: Tensor<WgpuBackend, 1> = bin_slice.reshape([bin_dims[1] as i32]);
        
        let cat_slice: Tensor<WgpuBackend, 2, Int> = all_cat_states.clone().slice([i..i+1, 0..cat_dims[1]]);
    ...
    let unnormalized: Vec<f32> = energies.iter()
