...
fn test_ising_energy() {
    ...
    let biases: Tensor<WgpuBackend, 1> = Tensor::from_data(vec![0.1f32, 0.2, 0.3].as_slice(), &device);
    ...
    //        = -(-0.8)
    //        = 0.8
    let state: Tensor<WgpuBackend, 1> = Tensor::from_data(vec![1.0f32, 0.0, 1.0].as_slice(), &device);
...
fn test_hinton_init() {
    ...
    let state_data: Vec<f32> = init_state[0].clone()
        .into_data()
        .to_vec()
        .expect("read state");
    
    let n_ones = state_data.iter().filter(|&&v| v > 0.5).count();
    println!("Number of 1s with positive bias: {}/{}", n_ones, state_data.len());
    
    // Should be mostly 1s (allow some variance from RNG)
    assert!(n_ones >= 7, "Expected mostly 1s with positive bias, got {}", n_ones);
}
...
fn test_ising_sampling_program_creation() {
    ...
    let edges: Vec<(Node, Node)> = nodes.windows(2)
    ...
    let result = IsingSamplingProgram::new(
        &model,
        free_blocks,
        clamped_blocks,
        &device,
    ...
    assert!(result.is_ok(), "IsingSamplingProgram creation should succeed: {:?}", result.err());
}
