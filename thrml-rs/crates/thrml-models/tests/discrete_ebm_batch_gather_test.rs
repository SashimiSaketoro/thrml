...
fn test_batch_gather_3d() {
    ...
    let data: Vec<f32> = result.clone().into_data().to_vec().expect("Failed to read tensor data");
    println!("Result: {:?}", data);
    ...
    for (i, (got, exp)) in data.iter().zip(expected.iter()).enumerate() {
        assert!((got - exp).abs() < 1e-5, "Result[{}] = {}, expected {}", i, got, exp);
    }
}
