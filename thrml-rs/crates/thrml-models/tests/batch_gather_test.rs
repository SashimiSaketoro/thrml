...
pub fn batch_gather_2d_gather(
    ...
    let indices_expanded: Tensor<WgpuBackend, 2, burn::tensor::Int> = indices.clone().unsqueeze_dim::<2>(1); // [batch] -> [batch, 1]
...
pub fn batch_gather_3d_gather(
    ...
    let indices0_2d: Tensor<WgpuBackend, 2, burn::tensor::Int> = indices0.clone().unsqueeze_dim::<2>(1); // [batch] -> [batch, 1]
...
pub fn batch_gather_2d_linear(
    ...
    let dim1_tensor = Tensor::from_data(
        vec![dim1_size as i32; batch_size].as_slice(),
        &device
    );
...
pub fn batch_gather_3d_linear(
    ...
    let stride0_tensor = Tensor::from_data(
        vec![stride0 as i32; batch_size].as_slice(),
        &device
    );
    let stride1_tensor = Tensor::from_data(
        vec![stride1 as i32; batch_size].as_slice(),
        &device
    );
    
    let linear_indices = batch_indices * stride0_tensor 
        + indices0.clone() * stride1_tensor 
        + indices1;
...
mod tests {
    ...
    fn test_batch_gather_2d_sequential_vs_linear() {
        ...
        let weights_1d: Tensor<WgpuBackend, 1> = Tensor::from_data(
            weights_data.as_slice(),
            &device
        );
        ...
        let data_gather: Vec<f32> = result_gather.clone().into_data().to_vec().expect("Failed to read tensor data");
        let data_lin: Vec<f32> = result_lin.clone().into_data().to_vec().expect("Failed to read tensor data");
        ...
        for (i, ((gath, lin), exp)) in data_gather.iter().zip(data_lin.iter()).zip(expected.iter()).enumerate() {
            println!("  [{}] gather={}, lin={}, exp={}", i, gath, lin, exp);
            assert!((gath - exp).abs() < 1e-5, "Gather result[{}] = {}, expected {}", i, gath, exp);
            assert!((lin - exp).abs() < 1e-5, "Linear result[{}] = {}, expected {}", i, lin, exp);
        }
        
        // Verify both approaches give same result
        assert_eq!(data_gather, data_lin, "Gather and linear approaches should match");
    }
    ...
    fn test_batch_gather_3d_sequential_vs_linear() {
        ...
        let indices0: Tensor<WgpuBackend, 1, burn::tensor::Int> = Tensor::from_data(
            vec![0i32, 1].as_slice(),
            &device
        );
        ...
        println!("Weights shape: {:?}", weights.dims());
        println!("Indices0 shape: {:?}, values: {:?}", indices0.dims(), vec![0i32, 1]);
        println!("Indices1 shape: {:?}, values: {:?}", indices1.dims(), vec![2i32, 3]);
        
        // Test gather approach
        let result_gather = batch_gather_3d_gather(weights.clone(), indices0.clone(), indices1.clone());
        println!("Gather result dims: {:?}", result_gather.dims());
        
        // Test linear
        let result_lin = batch_gather_3d_linear(weights.clone(), indices0.clone(), indices1.clone());
        println!("Linear result dims: {:?}", result_lin.dims());
        ...
        // Verify they match
        assert_eq!(data_gather, data_lin, "Gather and linear approaches should match for 3D");
        ...
        for (i, (gath, exp)) in data_gather.iter().zip(expected.iter()).enumerate() {
            assert!((gath - exp).abs() < 1e-5, "Result[{}] = {}, expected {}", i, gath, exp);
        }
    }
}
